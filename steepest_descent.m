function [x, history] = steepest_descent(f, grad_f, x0, max_iter, tolerance, mode)
    ...
    %   mode       - 'fixed', 'line_search' or 'armijo' depending on the
    ...
    for iter = 1:max_iter
        ...
        switch mode
            case 'fixed'
                gamma = 0.1;
            case 'line_search'
                phi = @(gamma) f(x + gamma * d); % Line search objective function
            ...
            case 'armijo'
                ...
                while f(x + gamma * d) > f(x) + gamma * alpha * (grad' * d)
